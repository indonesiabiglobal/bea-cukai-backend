...
type AuxiliaryMaterialReport struct {
...
type AuxiliaryMaterialReportRequest struct {
	TglAwal  time.Time `json:"tgl_awal" validate:"required"`
	TglAkhir time.Time `json:"tgl_akhir" validate:"required"`
	ItemCode string    `json:"item_code"`
	ItemName string    `json:"item_name"`
	Lap      string    `json:"lap" validate:"required"` // Dynamic item group parameter
	Page     int       `json:"page"`
	Limit    int       `json:"limit"`
}

// Response type alias for consistency
type AuxiliaryMaterialReportResponse = AuxiliaryMaterialReport
